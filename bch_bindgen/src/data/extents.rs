...
fn bkey_ptrs_raw(
    sc: &BkeyValSC<'_>,
) -> Option<(*const c::bch_extent_entry, *const c::bch_extent_entry)> {
    ...
    unsafe {
        match sc {
            ...
            BkeyValSC::stripe(_k, v) => Some((
                v.ptrs.as_ptr() as _,
                v.ptrs.as_ptr().add(v.nr_blocks as usize) as _,
            )),
...
pub fn bkey_extent_entries_sc<'a>(sc: &BkeyValSC<'a>) -> ExtentEntryIter<'a> {
    match bkey_ptrs_raw(sc) {
        Some((start, end)) => ExtentEntryIter {
            cur: start,
            end,
            _phantom: PhantomData,
        },
...
pub fn bkey_ptrs_sc<'a>(sc: &BkeyValSC<'a>) -> ExtentPtrIter<'a> {
    ExtentPtrIter {
        inner: bkey_extent_entries_sc(sc),
    }
}
